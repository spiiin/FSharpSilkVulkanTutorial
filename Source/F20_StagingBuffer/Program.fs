...
type HelloTriangleApplication() =
    ...
    member private this.CopyBuffer(srcBuffer: Buffer, dstBuffer: Buffer, size: uint64) =
        let mutable commandBuffer = Unchecked.defaultof<CommandBuffer>
        let mutable allocateInfo = CommandBufferAllocateInfo(
            SType = StructureType.CommandBufferAllocateInfo,
            Level = CommandBufferLevel.Primary,
            CommandPool = commandPool,
            CommandBufferCount = 1u
        )
        match vk.Value.AllocateCommandBuffers(device, &allocateInfo, &commandBuffer) with
        | Result.Success -> ()
        | _ -> raise <| Exception("failed to allocate command buffer!")

        let mutable beginInfo = CommandBufferBeginInfo(
            SType = StructureType.CommandBufferBeginInfo,
            Flags = CommandBufferUsageFlags.OneTimeSubmitBit
        )
        match vk.Value.BeginCommandBuffer(commandBuffer, &beginInfo) with
        | Result.Success -> ()
        | _ -> raise <| Exception("failed to begin command buffer!")

        let mutable copyRegion = BufferCopy(Size = size)

        vk.Value.CmdCopyBuffer(commandBuffer, srcBuffer, dstBuffer, 1u, &copyRegion)

        match vk.Value.EndCommandBuffer(commandBuffer) with
        | Result.Success -> ()
        | _ -> raise <| Exception("failed to end command buffer!")
        ...
        use submitInfoPtr = fixed &submitInfo

        match vk.Value.QueueSubmit(graphicsQueue, 1u, submitInfoPtr, Unchecked.defaultof<Fence>) with
        | Result.Success -> ()
        | _ -> raise <| Exception("failed to submit command buffer!")

        vk.Value.QueueWaitIdle(graphicsQueue) |> ignore
        vk.Value.FreeCommandBuffers(device, commandPool, 1u, &commandBuffer)
