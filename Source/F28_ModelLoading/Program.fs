...
[<StructLayout(LayoutKind.Sequential)>]
type Vertex =
    val mutable pos: Vector3D<float32>
    val mutable color: Vector3D<float32>
    val mutable texCoord: Vector2D<float32>

    new (pos: Vector3D<float32>, color: Vector3D<float32>, texCoord: Vector2D<float32>) = { pos = pos; color = color; texCoord = texCoord }
    ...
    static member GetAttributeDescriptions() =
        let attributeDescriptions = [|
            ...
            VertexInputAttributeDescription(
                ...
                Offset = uint32(Marshal.OffsetOf<Vertex>("texCoord"))
            )
        |]
...
type HelloTriangleApplication() =
    ...
    static let MODEL_PATH = @"viking_room.obj";
    static let TEXTURE_PATH = @"viking_room.png";
    ...
    let mutable depthImage = Unchecked.defaultof<Image>
    let mutable depthImageMemory = Unchecked.defaultof<DeviceMemory>
    let mutable depthImageView = Unchecked.defaultof<ImageView>
    ...
    let mutable vertices = Array.empty<Vertex>
    let mutable indices = Array.empty<uint>
    ...
    member private this.CleanUp() =
        match vk with
        | Some vkApi ->
            vkApi.DestroyImageView(device, depthImageView, NativePtr.nullPtr);
            vkApi.DestroyImage(device, depthImage, NativePtr.nullPtr);
            vkApi.FreeMemory(device, depthImageMemory, NativePtr.nullPtr);
    ...
    member private this.CreateImageViews() =
        ...
        for i in 0 .. swapChainImages.Length - 1 do
            swapChainImageViews.[i] <- this.CreateImageView(swapChainImages[i], swapChainImageFormat, ImageAspectFlags.ColorBit)

    member private this.CreateTextureImageView() =
        textureImageView <- this.CreateImageView(textureImage, Format.R8G8B8A8Srgb, ImageAspectFlags.ColorBit)
    ...
    member private this.CreateImageView(image: Image, format: Format, aspectFlags: ImageAspectFlags) =
        let mutable createInfo = ImageViewCreateInfo(
            SType = StructureType.ImageViewCreateInfo,
            Image = image,
            ViewType = ImageViewType.Type2D,
            Format = format,
            SubresourceRange = ImageSubresourceRange(aspectFlags, 0u, 1u, 0u, 1u)
        )
    ...
    member private this.CreateGraphicsPipeline() =
        ...
        let mutable depthStencil = PipelineDepthStencilStateCreateInfo(
    ...
    member private this.CreateRenderPass() =
        ...
        let mutable depthAttachment = AttachmentDescription(
        ...
        let mutable depthAttachmentRef = AttachmentReference(
            Attachment = 1u,
        ...
        use attachmentsPtr = fixed attachments
    ...
    member private this.CreateFramebuffers() =
        ...
        for i in 0 .. swapChainImageViews.Length - 1 do
            let mutable attachments = [| swapChainImageViews.[i]; depthImageView |]
            use attachmentsPtr = fixed attachments
    ...
    member private this.CreateCommandBuffers() =
        ...
        for i in 0 .. commandBuffers.Length - 1 do
            ...
            let mutable clearValues = [|
                ClearValue(Color = ClearColorValue(0f, 0f, 0f, 1f));
                ClearValue(DepthStencil = ClearDepthStencilValue(1f, 0u))
            |]
            use clearValuesPtr = fixed clearValues
            ...
            vk.Value.CmdBindIndexBuffer(commandBuffers.[i], indexBuffer, 0UL, IndexType.Uint32)
    ...
    member private this.CreateIndexBuffer() =
        let bufferSize = uint64 (sizeof<uint> * indices.Length)
        let mutable stagingBuffer = Unchecked.defaultof<Buffer>
        let mutable stagingBufferMemory = Unchecked.defaultof<DeviceMemory>
        this.CreateBuffer(bufferSize, BufferUsageFlags.TransferSrcBit, MemoryPropertyFlags.HostVisibleBit ||| MemoryPropertyFlags.HostCoherentBit, &stagingBuffer, &stagingBufferMemory)

        let mutable data: voidptr = NativePtr.toVoidPtr<uint>(NativePtr.nullPtr)
        vk.Value.MapMemory(device, stagingBufferMemory, 0UL, bufferSize, MemoryMapFlags.None, &data) |> ignore
        indices.AsSpan().CopyTo(Span<uint>(data, indices.Length))
        vk.Value.UnmapMemory(device, stagingBufferMemory)
    ...
    member private this.CreateDescriptorSetLayout() =
        ...
        let mutable samplerLayoutBinding = DescriptorSetLayoutBinding(
            Binding = 1u,
            DescriptorCount = 1u,
        ...
        let bindings = [| uboLayoutBinding; samplerLayoutBinding |]
        use bindingsPtr = fixed bindings
        let layoutInfo = DescriptorSetLayoutCreateInfo(
            SType = StructureType.DescriptorSetLayoutCreateInfo,
            BindingCount = uint32 bindings.Length,
    ...
    member private this.CreateDescriptorPool() =
        let poolSizes = [| 
            DescriptorPoolSize(Type = DescriptorType.UniformBuffer, DescriptorCount = uint32 swapChainImages.Length)
            DescriptorPoolSize(Type = DescriptorType.CombinedImageSampler, DescriptorCount = uint32 swapChainImages.Length)
        |]
        use poolSizesPtr = fixed poolSizes
        let mutable poolInfo = DescriptorPoolCreateInfo(
            SType = StructureType.DescriptorPoolCreateInfo,
            PoolSizeCount = uint32 poolSizes.Length,
    ...
    member private this.CreateDescriptorSets() =
        ...
        match vk.Value.AllocateDescriptorSets(device, &allocateInfo, descriptorSetsPtr) with
        | Result.Success -> ()
        ...
        for i in 0 .. swapChainImages.Length - 1 do
            ...
            let mutable imageInfo = DescriptorImageInfo(
            ...
            |]
            use descriptorWritesPtr = fixed descriptorWrites
            vk.Value.UpdateDescriptorSets(device, uint32 descriptorWrites.Length, descriptorWritesPtr, 0u, NativePtr.nullPtr)

    member private this.CreateTextureImage() =
        use img = SixLabors.ImageSharp.Image.Load<SixLabors.ImageSharp.PixelFormats.Rgba32>(TEXTURE_PATH)
    ...
    member private this.CreateDepthResources() =
        let mutable depthFormat = this.FindDepthFormat()
        this.CreateImage(
        ...
        depthImageView <- this.CreateImageView(depthImage, depthFormat, ImageAspectFlags.DepthBit)

    member private this.FindSupportedFormat(candidates: Format seq, tiling: ImageTiling, features: FormatFeatureFlags) =
        candidates
        |> Seq.tryFind (fun format ->
            let mutable props = FormatProperties()
            vk.Value.GetPhysicalDeviceFormatProperties(physicalDevice, format, &props)

            match tiling with
            | ImageTiling.Linear when (props.LinearTilingFeatures &&& features) = features -> true
            | ImageTiling.Optimal when (props.OptimalTilingFeatures &&& features) = features -> true
            | _ -> false
        )
        |> function
            | Some format -> format
            | None -> raise <| Exception("failed to find supported format!")

    member private this.FindDepthFormat() =
        this.FindSupportedFormat([| Format.D32Sfloat; Format.D32SfloatS8Uint; Format.D24UnormS8Uint |], ImageTiling.Optimal, FormatFeatureFlags.DepthStencilAttachmentBit)

    member private this.LoadModel() =
        use assimp = Assimp.GetApi()
        let scenePtr = assimp.ImportFile(MODEL_PATH, uint32 PostProcessPreset.TargetRealTimeMaximumQuality)
        let scene = NativePtr.read scenePtr

        let vertexMap = System.Collections.Generic.Dictionary<Vertex, uint32>()
        let mutable assimpVertices = System.Collections.Generic.List<Vertex>()
        let mutable assimpIndices = System.Collections.Generic.List<uint32>()

        let rec visitSceneNode (nodePtr: nativeptr<Node>) =
            let node = NativePtr.read nodePtr
            for m in 0 .. (int node.MNumMeshes) - 1 do
                let nodeMMeshesIndex = NativePtr.get node.MMeshes m
                let meshPtr =  NativePtr.get scene.MMeshes (int nodeMMeshesIndex)
                let mesh = NativePtr.read meshPtr
                for f in 0 .. (int mesh.MNumFaces) - 1 do
                    let face = NativePtr.get mesh.MFaces f
                    for i in 0 .. (int face.MNumIndices) - 1 do
                        let index = NativePtr.get face.MIndices i
                        let position = NativePtr.get mesh.MVertices (int index)
                        let texCoords0 = mesh.MTextureCoords.[0]
                        let texture = NativePtr.get mesh.MTextureCoords.[0] (int index)

                        let vertex = Vertex(
                            Vector3D<float32>(position.X, position.Y, position.Z),
                            Vector3D<float32>(1.0f, 1.0f, 1.0f),
                            Vector2D<float32>(texture.X, 1.0f - texture.Y)  // Flip Y for OBJ in Vulkan
                        )

                        match vertexMap.TryGetValue(vertex) with
                        | true, meshIndex -> assimpIndices.Add(meshIndex)
                        | false, _ ->
                            assimpIndices.Add(uint32 assimpVertices.Count)
                            vertexMap.[vertex] <- uint32 assimpVertices.Count
                            assimpVertices.Add(vertex)

            for c in 0 .. (int node.MNumChildren) - 1 do
                visitSceneNode <| NativePtr.get node.MChildren c

        visitSceneNode scene.MRootNode
        assimp.ReleaseImport scenePtr

        vertices <- assimpVertices.ToArray()
        indices <- assimpIndices.ToArray()
