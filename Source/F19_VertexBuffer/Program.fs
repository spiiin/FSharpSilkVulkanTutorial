...
type Vertex =
    ...
    val mutable color: Vector3D<float32>
     ...
     new (pos: Vector2D<float32>, color: Vector3D<float32>) =
            { pos = pos; color = color }
...
type HelloTriangleApplication() =
    ...
    let mutable vertexBuffer: Buffer = Unchecked.defaultof<Buffer>
    let mutable vertexBufferMemory: DeviceMemory = Unchecked.defaultof<DeviceMemory>
    ...
    let vertices = [| 
        Vertex(Vector2D(0.0f, -0.5f), Vector3D(1.0f, 0.0f, 0.0f))
    ...
    member private this.CreateCommandBuffers() =
        ...
        for i in 0 .. commandBuffers.Length - 1 do
            ...
            vk.Value.CmdDraw(commandBuffers.[i], uint32(vertices.Length), 1u, 0u, 0u)
    ...
    member private this.CreateVertexBuffer() =
        let mutable bufferInfo = BufferCreateInfo(
            ...
            Size = uint64(sizeof<Vertex> * vertices.Length),
            Usage = BufferUsageFlags.VertexBufferBit,
            SharingMode = SharingMode.Exclusive
        )

        use vertexBufferPtr = fixed &vertexBuffer
        match vk.Value.CreateBuffer(device, &bufferInfo, NativePtr.nullPtr, vertexBufferPtr) with
        | Result.Success -> ()
        | _ -> raise <| Exception("failed to create vertex buffer!")

        let mutable memRequirements = MemoryRequirements()
        vk.Value.GetBufferMemoryRequirements(device, vertexBuffer, &memRequirements)

        let mutable allocateInfo = MemoryAllocateInfo(
            SType = StructureType.MemoryAllocateInfo,
            AllocationSize = memRequirements.Size,
            MemoryTypeIndex = this.FindMemoryType(memRequirements.MemoryTypeBits, MemoryPropertyFlags.HostVisibleBit ||| MemoryPropertyFlags.HostCoherentBit)
        )

        use vertexBufferMemoryPtr = fixed &vertexBufferMemory
        match vk.Value.AllocateMemory(device, &allocateInfo, NativePtr.nullPtr, vertexBufferMemoryPtr) with
        | Result.Success -> ()
        | _ -> raise <| Exception("failed to allocate vertex buffer memory!")

        vk.Value.BindBufferMemory(device, vertexBuffer, vertexBufferMemory, 0UL) |> ignore

        let mutable data: voidptr = NativePtr.toVoidPtr<Vertex>(NativePtr.nullPtr)
        vk.Value.MapMemory(device, vertexBufferMemory, 0UL, bufferInfo.Size, MemoryMapFlags.None, &data) |> ignore
        let span = Span<Vertex>(data, vertices.Length * sizeof<Vertex>)
        vertices.AsSpan().CopyTo(span)
        vk.Value.UnmapMemory(device, vertexBufferMemory)
